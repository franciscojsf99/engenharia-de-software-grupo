...
public class XMLQuestionExportVisitor implements Visitor {
    ...
    public void visitQuestionDetails(ItemCombinationQuestion question) {
        ...
        Element optionsElement = new Element("combinationOptions");
        this.currentElement.addContent(optionsElement);

        Element combinationsElement = new Element("combinations");
        this.currentElement.addContent(combinationsElement);

        this.currentElement = optionsElement;
        question.visitOptions(this);

        this.currentElement = combinationsElement;
        question.visitCombinations(this);
    }
    ...
    @Override
    public void visitCombinationOption(CombinationOption option) {
        Element combinationOptionElement = new Element("combination_option");

        combinationOptionElement.setAttribute("content", option.getContent());
        combinationOptionElement.setAttribute("column", String.valueOf(option.getType()));

        this.currentElement.addContent(combinationOptionElement);
    }

    @Override
    public void visitCombination(Combination combination) {
        Element combinationsElementRoot = this.currentElement;

        Element combinationElement = new Element("combination");
        this.currentElement.addContent(combinationElement);
        this.currentElement = combinationElement;
        combination.visitCombinationOptions(this);

        this.currentElement = combinationsElementRoot;
    }
    ...
    public void visitOption(Option option) {
        ...
        optionElement.setAttribute("importance", String.valueOf(option.getImportance()));
        optionElement.setAttribute(SEQUENCE, String.valueOf(option.getSequence()));
        optionElement.setAttribute(CONTENT, option.getContent());
        optionElement.setAttribute("correct", String.valueOf(option.isCorrect()));
