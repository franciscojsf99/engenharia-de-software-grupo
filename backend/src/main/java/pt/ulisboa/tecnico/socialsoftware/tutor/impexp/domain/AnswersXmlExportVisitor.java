...
public class AnswersXmlExportVisitor implements Visitor {
    ...
    public void visitAnswerDetails(ItemCombinationAnswer answer) {
        ...
        if(answer.isAnswered()) {
            Element combinationsContainerElement = new Element("combinations");
            answer.getCombinations().forEach(combination -> {
                Element combinationElement = new Element("combination");

                combinationElement.setAttribute("leftCombinationOption", String.valueOf(combination.getLeftOption().getContent()));
                combinationElement.setAttribute("rightCombinationOption", String.valueOf(combination.getRightOption().getContent()));
                combinationsContainerElement.addContent(combinationElement);
            });
            this.currentElement.addContent(combinationsContainerElement);
        }
    }
}
