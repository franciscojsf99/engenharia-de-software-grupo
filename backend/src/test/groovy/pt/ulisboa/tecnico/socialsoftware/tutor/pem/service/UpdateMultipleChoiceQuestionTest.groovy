...
class UpdateMultipleChoiceQuestionTest extends SpockTest {
    ...
    def "update question with two options true"() {
        ...
        when:
        questionService.updateQuestion(question.getId(), questionDto)
        // changed this test because the extended functionality allows two right options

        then: "now two correct answers"
        questionDto.getQuestionDetailsDto().getNumCorrectAnswers() == 2
    }
