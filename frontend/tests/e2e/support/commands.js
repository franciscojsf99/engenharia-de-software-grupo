...
Cypress.Commands.add(
  ...
  cy.get('[data-cy="newQuestionButton"]').click({force: true});

  cy.get('[data-cy="questionTypeInput"]')
      .type('open_answer', {force: true})
      .click({force: true});

  //creates an open answer question
  ...
  cy.get('[data-cy="questionAnswerInput"').type(answer, {force: true,});
  }
);

Cypress.Commands.add(
  'confirmOpenAnswerQuestion',
  (title, question, answer) => {
  cy.get('[data-cy="questionTitleTextArea"]')
    ...
    cy.get('[data-cy="questionAnswerInput"]').should(
      'have.value',
      answer
    );
  }
);
Cypress.Commands.add('setLeftOptions', (length) => {
    cy.get('[data-cy="questionOptionsInput"]')
        .should('have.length', length)
        .each(($el, index, $list) => {
            cy.get($el).within(($ls) => {
                cy.get(`[data-cy="LeftOption${index + 1}"]`).type(`Left Option ${index + 1}`);
            });
        });
});

Cypress.Commands.add('setRightOptions', (length) => {
    cy.get('[data-cy="ManageRightOptionsInput"]')
        .should('have.length', length)
        .each(($el, index, $list) => {
            cy.get($el).within(($ls) => {
                cy.get(`[data-cy="RightOption${index + 1}"]`).type(`Right Option ${index + 1}`);
            });
        });
});
...
Cypress.Commands.add('teacherCreateItemCombinationQuestionAndQuiz', () => {
  cy.server();
  ...
  cy.get('[data-cy="quizTitleTextArea"]').type('Cypress Quiz Example - 01', { force: true });
  cy.get('#availableDateInput-input').click();
  cy.get(
      '.datetimepicker > .datepicker > .datepicker-buttons-container > .datepicker-button > .datepicker-button-content'
  ).click();
  cy.get('[data-cy=addToQuizButton]').click({ force: true });
  cy.get('button').contains('Save').click();
});
