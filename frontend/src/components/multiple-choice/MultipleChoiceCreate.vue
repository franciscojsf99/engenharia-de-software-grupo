<template>
  <div class="multiple-choice-options">
    ...
    >
      ...
      <v-col cols="1">
        <v-switch v-model="option.correct" :data-cy="`Switch${index + 1}`" />
      </v-col>
      <v-col
        v-if="
          option.correct &&
          sQuestionDetails.options.filter((c) => c.correct).length > 1
        "
        cols="10"
      >
        <v-textarea
          v-model="option.importance"
          :label="`Option ${index + 1} importance`"
          :data-cy="`Option${index + 1} importance`"
          rows="1"
          auto-grow
        ></v-textarea>
      </v-col>
